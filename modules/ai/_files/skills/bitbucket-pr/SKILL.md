...
> **Agent sandbox limitation in this environment:** `bb` does not work reliably inside agent sandboxes. This also affects the wrappers that invoke it. For sandbox failures, run the same command through an approved execution mechanism **outside the sandbox**, in a normal shell with access to the usual configuration and credentials. Starting another `zsh` inside the sandbox does not remove its restrictions. Preserve the target repository, working directory, and existing authorization requirements. If outside-sandbox execution is unavailable or denied, give the user the exact command and working directory to run in their terminal; do not bypass the restriction. Do not change credentials merely because the sandboxed call failed.
...
echo "Closes #999." | \
  ${CLAUDE_SKILL_DIR}/scripts/bitbucket_pr.sh create \
    --reviewer 557058:0c8e… --reviewer '{00000000-0000-4000-8000-000000000000}' \
...
${CLAUDE_SKILL_DIR}/scripts/bitbucket_pr.sh update 1234 \
  --add-reviewer 557058:0c8e… --remove-reviewer 1234567890abcdef12345678
...
| `bb` or its wrapper fails inside an agent sandbox | Known sandbox limitation in this environment | Use approved outside-sandbox execution, or hand the command to the user for their terminal; see the sandbox callout above. A nested `zsh` does not help. |
| Auth / 401 errors | May be a transient Bitbucket outage; persistent failures may involve the profile or token | Wait briefly and retry before investigating credentials. Before retrying a write with an uncertain outcome, read back its state to avoid duplicates. If the error persists, check the execution environment and `BB_PROFILE`, then investigate credentials; do not immediately recreate the profile. |
...
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh id JIRA-2978              # → 123456
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh id JIRA-2978 --format json # → {id,key,summary}
...
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh prs JIRA-2978
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh prs JIRA-2978 --state MERGED   # client-side filter
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh prs JIRA-2978 --format tsv      # id, status, source, dest, repo, url, title
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh prs JIRA-2978 --no-resolve-repo # skip the extra repository call (raw UUID URL)

# Linked branches and repositories
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh branches JIRA-2978
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh repos    JIRA-2978              # prints the <workspace>/<slug>
...
${CLAUDE_SKILL_DIR}/scripts/bitbucket_jira.sh prs JIRA-2978 --format tsv
#    42   OPEN   feature/x   master   acme/example-service   https://…/pull-requests/42   …

# 2. Read its comments / tasks from anywhere, targeting that repo explicitly
${CLAUDE_SKILL_DIR}/scripts/bitbucket_pr_comments.sh list 42 --repo acme/example-service
${CLAUDE_SKILL_DIR}/scripts/bitbucket_pr_tasks.sh    list 42 --repo acme/example-service
```
...
`557058:0c8e…` form or a 24-hex legacy id (`1234567890abcdef12345678`) — or a
...
${CLAUDE_SKILL_DIR}/scripts/bitbucket_pr_reviewers.py set --pr 1234 --repo "$R" \
  --add 557058:0c8e… --remove 1234567890abcdef12345678
