...
license: Vibecoded
allowed-tools: Read(references/*) Bash(tmux:*) Bash(scripts/find-sessions.sh:*) Bash(scripts/wait-for-text.sh:*)
---
...
- **lldb**: `tmux -S "$SOCKET" send-keys -t target -- 'lldb ./a.out' Enter`; wait for `(lldb)`; set breakpoints, run, inspect with `bt`, `frame variable`, etc.; exit via `quit`.
- **gdb**: `tmux -S "$SOCKET" send-keys -t target -- 'gdb --quiet ./a.out' Enter`; disable paging `set pagination off`; break with `C-c`; issue `bt`, `info locals`, etc.; exit via `quit` then confirm `y`.
...
`./scripts/wait-for-text.sh` polls a pane for a regex (or fixed string) with a timeout. Works on Linux/macOS with zsh + tmux + grep.
...
`./scripts/find-sessions.sh` lists tmux sessions with metadata. Supports filtering by name and scanning all sockets.

```bash
./scripts/find-sessions.sh -S "$SOCKET"           # list sessions on a specific socket
./scripts/find-sessions.sh --all                   # scan all sockets under CLAUDE_TMUX_SOCKET_DIR
./scripts/find-sessions.sh -S "$SOCKET" -q python  # filter by name
```
