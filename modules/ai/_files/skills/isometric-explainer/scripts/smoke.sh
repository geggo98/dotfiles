...
if [ -n "$BASH_VERSION" ]; then
  ...
  echo >&2 "Run it directly (./scripts/smoke.sh) or with: zsh scripts/smoke.sh"
  exit 1
fi
set -euo pipefail
...
# Playwright resolves browsers by EXACT revision, so the nix bundle's
# playwright-driver version MUST equal PW_VERSION below. This pin is coupled
# with ../../slidev/scripts/check-slide-overflow.sh — bump both (and
# NIXPKGS_REF) in lockstep.
PW_VERSION="1.58.2"
...
if command -v nix >/dev/null 2>&1; then
  ...
  if [ "$nixpw" != "$PW_VERSION" ]; then
    echo >&2 "smoke.sh: nixpkgs playwright-driver ('${nixpw}') != checker pin ('${PW_VERSION}'); bump both in lockstep. Falling back to CDN browsers."
...
if [ "$provisioned" -eq 0 ]; then
  echo >&2 "smoke.sh: using Playwright CDN browsers (nix bundle unavailable); ensuring chromium…"
  deno run --allow-env --allow-read --allow-write --allow-net --allow-run --allow-sys \
    npm:playwright@"${PW_VERSION}" install chromium \
    || echo >&2 "smoke.sh: 'playwright install' returned non-zero; continuing with whatever installed."
fi
