{ ... }:
{
  flake.modules.darwin.macos = { config, pkgs, ... }:
    let
      # IoskeleyMono — open-source Iosevka build that mimics commercial Berkeley Mono,
      # patched with Nerd Font glyphs. Not in nixpkgs; fetched from upstream release.
      ioskeleyMonoNerdFont = pkgs.stdenvNoCC.mkDerivation (finalAttrs: {
        ...
        meta = with pkgs.lib; {
          description = "Iosevka build mimicking Berkeley Mono, patched with Nerd Font glyphs";
