{ ... }:
{
  flake.modules.homeManager.vault = { config, pkgs, lib, ... }:
    let
      ...
      # finds it.
      #
      # writeLegacyToken: additionally mirror the fresh token into the legacy
      ...
      # for tools that hardcode Path.of(user.home, ".vault-token"). Only the
      # staging login sets this, so a production token never lands there. May
      # go stale after a helper `erase` — accepted for a legacy shim.
      mkVaultLogin = { name, addrFile, writeLegacyToken ? false }:
        let
          legacyTokenSnippet = lib.optionalString writeLegacyToken ''
        ...
        pkgs.writeShellApplication {
          ...
          text = ''
            ...
            # Force the address: interactive fish exports VAULT_ADDR (staging
    ...
    in
    {
      ...
      # Default plain `vault` to the staging instance. Reads the secret at
