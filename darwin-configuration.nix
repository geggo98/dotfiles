...
let
  ...
  node = import ./node/default.nix { nodejs = pkgs.nodejs-14_x; }; # Update with: nix shell nixpkgs#node2nix --command node2nix -i ./node/node-packages.json -o ./node/node-packages.nix
